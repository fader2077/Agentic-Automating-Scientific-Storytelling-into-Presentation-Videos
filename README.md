...
The requested duration is treated as a pacing target, not a hard fill requirement; outputs may finish up to about one minute short to preserve natural speech. The planner uses a smaller academic slide budget for medium videos, so a 6-minute request is not forced into a 24-page deck. For longer videos, the pipeline expands slide narration before TTS, lowers F5TTS voice speed, and inserts short sentence pauses before slide-level concatenation. After synthesis it applies an ffmpeg limiter with headroom before any duration handling. If narration is shorter than the accepted lower bound, it distributes short pauses across slide audio instead of stretching speech and causing robotic artifacts; small `atempo` correction is reserved for modest over-target cases. Burned-in subtitles are one or two readable slide captions that span the slide duration, while the full narration remains available in the script and SRT artifacts.
